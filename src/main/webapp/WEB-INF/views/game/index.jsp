        ...
        $('#search_date, #search_sport').on('change', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    var content = '';
                    if (data.length === 0) {
                        content += '<tr><td colspan="15">등록된 종목이 없습니다.</td></tr>';
                    } else {
                        $.each(data, function(index, vo) {
                            content += '<div>';
                            content += '<img src="' + vo.sport_pictogram + '" alt="">';
                            if (vo.country1_flag) {
                                content += '<img src="' + vo.country1_flag + '" alt="">';
                            } else {
                                content += '<div id="unknown_flag">?</div>';
                            }
                            if (vo.country2_flag) {
                                content += '<img src="' + vo.country2_flag + '" alt="">';
                            } else {
                                content += '<div id="unknown_flag">?</div>';
                            }
                            ...
                            content += '<button onclick="redirectToDetail(' + vo.game_id + ')">Detail</button>';
                            content += '</div>';
                        });
                    }
                    $('section > div:last-child').html(content);
                },
